...
Note that at the time of data collection, our participants sat 60cm from the screen (the distance at whick the Eye Tribe reports having 0.1 degrees visual angle root mean squared error) and our Psychopy monitor center was set to the original default settings (including the assumed 57cm viewing distance). Thus, to replicate the experiment exactly, you may: use our exact code and place participants 60cm from the screen while keeping the default monitor settings, or, if you wish for participants to view the stimuli from a different distance, change the stimulus sizes (in `sustained_attention_experiment/attention_memory.py` and `variable_attention_experiment/attention_memory.py`) according to the values reported in the manuscript.
