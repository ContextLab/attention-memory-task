...
class RatingScale_kz(MinimalStim):
    """A class for obtaining ratings, e.g., on a 1-to-7 or categorical scale.

    A RatingScale_kz instance is a re-usable visual object having a ``draw()``
    ...
    The Builder RatingScale_kz component gives a restricted set of options,
    but also allows full control over a RatingScale_kz via the
    'customize_everything' field.

    A RatingScale_kz instance has no idea what else is on the screen.
    ...
    **Example 1**:
        ...
        A basic 7-point scale::
            ...
            RatingScale_kz = visual.RatingScale_kz(win)
            item = <statement, question, image, movie, ...>
            while RatingScale_kz.noResponse:
                item.draw()
                RatingScale_kz.draw()
                win.flip()
            rating = RatingScale_kz.getRating()
            decisionTime = RatingScale_kz.getRT()
            choiceHistory = RatingScale_kz.getHistory()
    ...
    **Example 2**:
        ...
        and not need a mouse::
            ...
            RatingScale_kz = visual.RatingScale_kz(
    ...
    **Example 3**:
    ...
    For other examples see Coder Demos -> stimuli -> RatingScale_kz.py.
    ...
    """
        ...
        super(RatingScale_kz, self).__init__(name=name, autoLog=False)
        ...
        if obsArgs:
            msg = ('RatingScale_kz obsolete args: %s; see changelog v1.80.00'
        ...
        if unknownArgs:
            msg = "RatingScale_kz unknown kwargs: %s"
    ...
    def _initFirst(self, showAccept, mouseOnly, noMouse, singleClick,
        ...
        if not self.showAccept:
            ...
            if self.mouseOnly and not self.singleClick:
                ...
                msg = ("RatingScale_kz %s: ignoring mouseOnly (because "
        ...
        if choices and len(list(choices)) < 2:
            msg = "RatingScale_kz %s: choices requires 2 or more items"
        ...
        if marker == 'hover' and not self.choices:
            logging.error("RatingScale_kz: marker='hover' requires "
                          "a set of choices.")
            core.quit()
        ...
        if tickMarks and not labels is False:
            ...
            if len(self.labelTexts) != len(tickMarks):
                msg = "RatingScale_kz %s: len(labels) not equal to len(tickMarks)"
                logging.warning(msg % self.name)
                self.labelTexts = tickMarks
    ...
    def _initPosScale(self, pos, size, stretch, log=True):
        ...
        if pos:
            if len(list(pos)) == 2:
                ...
                msg = "RatingScale_kz %s: pos expects a tuple (x,y)"
                logging.warning(msg % self.name)
        try:
    ...
    def _initKeys(self, acceptKeys, skipKeys, leftKeys, rightKeys, respKeys):
        ...
        if respKeys and hasattr(respKeys, '__iter__'):
            ...
            if set(self.respKeys).intersection(nonRespKeys):
                msg = 'RatingScale_kz %s: respKeys may conflict with other keys'
                logging.warning(msg % self.name)
        else:
    ...
    def _initMarker(self, marker, markerColor, expansion):
        ...
        if isinstance(marker, basestring):
            ...
            logging.error("RatingScale_kz: custom marker has no draw() method")
            ...
            if hasattr(marker, 'pos'):
                ...
                logging.error(
                    "RatingScale_kz: custom marker has no pos attribute")
    ...
    def setDescription(self, scale=None, log=True):
        ...
        Useful when using the same RatingScale_kz object to rate several
        ...
        if log and self.autoLog:
            logging.exp('RatingScale_kz %s: setDescription="%s"' %
    ...
    def setMarkerPos(self, tick):
        ...
        Assuming you have defined rs = RatingScale_kz(...), you can specify
    ...
    def acceptResponse(self, triggeringAction, log=True):
        ...
        if log and self.autoLog:
            ...
            logging.data('RatingScale_kz %s: (%s) rating=%s' % vals)
    ...
    def draw(self, log=True):
        ...
        if not self.noResponse and self.decisionTime == 0:
            ...
            if log and self.autoLog:
                logging.data('RatingScale_kz %s: rating RT=%.3f' %
                             (self.name, self.decisionTime))
                logging.data('RatingScale_kz %s: history=%s' %
    ...
    def reset(self, log=True):
        ...
        # RatingScale_kz instance is used by a subject
        ...
        if log and self.autoLog:
            logging.exp('RatingScale_kz %s: reset()' % self.name)
