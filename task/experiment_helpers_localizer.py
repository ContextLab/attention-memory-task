...
# Tiny helpers
def results(subject, df):
    for trial in df.index.values:
        start_time = df.loc[trial, 'Stimulus Onset']
        stop_time = df.loc[trial, 'Stimulus Onset']
        for eyedata_file in range(20):
            file = open(subject + "/eye_data/" + subject[0] + "_" + eyedata_file, "w")
            if file == None:
                break
...
def initialize_df(info, categories, paths, params):
    ...
    localizer_columns = ['Run', 'Trial','Image']

    localizer_df = pd.DataFrame(index = range(len(localizer)*2), columns=localizer_columns)
    localizer_df['Run'],localizer_df['Trial'], localizer_df['Image'] = localizer_setup(params, localizer_face, localizer_place)
...
def localizer_setup(params, face, place):
    run = []
    trial = []
    image = []
    for x in range(params['localizer_runs']):
        trial.extend(['Face']*params['localizer_runs'])
        image.extend(face[:10])
        del face[:10]
        trial.extend(['Place']*params['localizer_runs'])
        image.extend(place[:10])
        del place[:10]
        run.extend([x]*(params['localizer_runs']*2))
    return([run, trial, image])
...
def rating_pull(rating_tuple):
    ...
    if len(rating_tuple)>1:
        ...
        rt = rating_tuple[0][1]
    return(rating, rt)
# def localizer_run(win, run, localizer_df, params, timing, paths, block):


def localizer_run(win, localizer_df, params, timing, paths, info):
    """
    Displays a full localizer run
    inputs:
        win - visual window
        run - run number (int)
        localizer_df - all trial infor for current localizer block (dataframe)
        params - experiment parameters (dictionary)
    ...
    for x in range(0,10):
        step+=10
        print(x)
        new_faces = localizer_df['Image'][step-10:step]
        localizer_faces[str(x)] = new_faces
        step+=10
        print("==================")
        new_places = localizer_df['Image'][step-10:step]
        localizer_places[str(x)] = new_places



    fixation = fix_stim(win)
    # for trial in range(params['localizer_runs']):
    if picture == "f":
        for key, value in localizer_faces[block].items():
            fixation.setAutoDraw(True)
            display(win, [fixation], timing['pause'], path = paths)
            image = memory_stim(win, value, paths['stim_path'])
            display(win, [image], timing['mem'], accepted_keys=None, df=localizer_df, path = paths)
            fixation.setAutoDraw(False)
    elif picture == "p":
        for key, value in localizer_places[block].items():
            fixation.setAutoDraw(True)
            display(win, [fixation], timing['pause'], path = paths)
            image = memory_stim(win, value, paths['stim_path'])
            display(win, [image], timing['mem'], accepted_keys=None, df=localizer_df, path = paths)
